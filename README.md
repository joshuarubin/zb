...
* Can complement other build systems like `make`
...
* `go generate` may be called before building the package with the `//go:generate` and `//zb:generate` annotations
* `main` packages (commands) are called with extra linker flags that cause `main.gitCommit` and `main.buildDate` variables to be set if they exist. See [`zb/main.go`](https://github.com/joshuarubin/zb/blob/master/main.go) as an example of how to utilize this.
* Executes `go install` for each stale package it finds and will execute concurrent `go install` processes when the dependency allows. Concurrency can be limited with `$GOMAXPROCS`.
...
Honors all `go test` flags except those intended to be passed directly to the test binary.

Use the `-f` flag to treat the test results as uncached, forcing them to be executed (and cached) again.
...
Lists the absolute paths where each of the `main` packages in the repositories will be built
...
Modify the base directory for storing results of commands that cache their results (`test` and `lint`).
...
* Complete all `godoc` documentation
* Add comprehensive testing
* Detect import cycles in dependency calculation
* Wrap [`govendor`](https://github.com/kardianos/govendor) in an opinionated way
